...
<img width="1592" height="527" alt="Screenshot 2026-09-10 235234" src="https://github.com/user-attachments/assets/1477149f-1a1d-4585-9644-61938815e184" />
...
Although `data.txt` is ASCII text, its contents are actually a **hexdump**.

We need to convert this hexdump back into the original binary file.

---

## 📁 Step 6 — Create a Temporary Directory

The level recommends creating a directory under `/tmp`.
...
> ⚠️ Your directory name will be different.
...
creates a temporary directory with a randomly generated name.

---

## 📋 Step 7 — Copy `data.txt`

Suppose the temporary directory created was:
...
> ⚠️ Replace `/tmp/tmp.abc123/` with the actual directory created by `mktemp -d`.
...
Now convert the hexdump back into the original binary file.
...
`xxd` can create hexadecimal dumps and can also convert hexadecimal data back into binary.
...
This tells us that the file is compressed using **gzip**.

---

## 🗜️ Step 11 — Rename the File

Rename the file with the `.gz` extension:
...
decompresses the gzip file and produces:
...
This means the next compression format is **bzip2**.
...
The file is compressed using gzip again.
...
Now the output should indicate:
...
So `data5.bin` is another TAR archive.
...
This is one of the most important commands in this level because the file format changes after each decompression or extraction.
...
After every decompression or extraction, we need to know what the next format is.
...
It can identify formats such as:
